...
public class Mace extends Application {
	...
	Sinhreonizer sinh = new Sinhreonizer();
	int id = 0;
	ConcurrentHashMap<Integer, Integer> mapaTrecaIgr = new ConcurrentHashMap<>();
 	
	class Sinhreonizer {
		
		Semaphore igracke[] = new Semaphore[7];
		Semaphore mutex = new Semaphore(1);
		
		{
			for (int i = 0; i < igracke.length; i++) {
				igracke[i] = new Semaphore(1);
			}
		}
		
		public void zauzmi(int id) throws InterruptedException {
			boolean ok = false;
			do {
				try {
					mutex.acquire();
					boolean lUspesna = igracke[leva(id)].tryAcquire();
					boolean dUspesna = igracke[desna(id)].tryAcquire();

					boolean treca = false;
					
					int i = 0;
					while (i < igracke.length && !treca) {
						if (i != leva(id) && i != desna(id)) {
							treca = igracke[i].tryAcquire();
							if (!treca)
								i++;							
						} else {
							i++;
		...
		private int desna(int id) {
			if (id == 0) {
				return igracke.length - 1;
			} else {
				return id - 1;
			}
		}
		
		public void oslobodi(int id) {
			igracke[leva(id)].release();
			igracke[desna(id)].release();
			int getIgracka = mapaTrecaIgr.get(id);
			igracke[getIgracka].release();
			mapaTrecaIgr.remove(id);
		}	
	}
